...
def main(args):
    if args["scene_names"] == "all":
        ...
        scene_names = [
            ii.replace('.json','').replace('_bbox','').replace('_scene','') for ii in os.listdir(args["scene_json_root"])
    ...
    for scene in progressbar(scene_names):
        ...
        if not os.path.exists(json_path):
            print('NOT EXIST')
            continue
            # json_path = os.path.join(args["scene_json_root"], f"m_{scene}.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--screenshot_root", default='./all_images/', help="Folder with scene screenshots"
    )
    parser.add_argument(
        "--scene_json_root", default='./simmc2_scene_jsons_dstc10_public/public/', help="Folder with scene JSONs"
    )
    parser.add_argument(
        "--save_root", default='./bbox_vis_out/', help="Folder to save the screenshots w/ bboxes"
    )
    parser.add_argument(
        "--scene_names",
        nargs="+",
